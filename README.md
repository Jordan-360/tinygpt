...
<img width="1200" height="675" alt="shakespeare_loss" src="https://github.com/user-attachments/assets/3b7c1c24-fefc-4240-bb52-90050f1af632" />
...
Tokenizer: each unique character in the dataset is mapped to an integer ID.
Embeddings: each ID, plus its position in the sequence, becomes a 384-dimensional vector.
Transformer blocks (x6): each block uses causal multi-head self-attention (6 heads), so every character can draw on the context before it, followed by a feed-forward network. Residual connections and layer normalization keep training stable.
Output layer: converts the final vectors into a probability for every possible next character.
Training: cross-entropy loss on next-character prediction, optimized with AdamW. Mixed-precision (bfloat16) training speeds things up on the GPU. The checkpoint with the best validation loss is kept.
Generation: the model samples a character, appends it, and repeats. A temperature setting controls how adventurous the sampling is.
Setting	Value
Layers	6
Attention heads	6
Embedding size	384
...
Requires Python 3.10+ and a PyTorch install with CUDA (pytorch.org). It also runs on CPU, just much more slowly.
...
requirements.txt
What I learned
<!-- TODO: write this in your own words after your runs. This is the section interviewers read. Ideas: - what surprised you watching the samples improve - what train vs. validation loss told you (e.g. signs of overfitting) - a problem you hit and how you solved it - what changing a setting (temperature, model size, context length) did -->
Next steps
...
The architecture follows Andrej Karpathy's nanoGPT and his "Let's build GPT: from scratch, in code, spelled out" lecture. Tiny Shakespeare dataset from char-rnn.
